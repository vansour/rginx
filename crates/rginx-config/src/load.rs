...
fn parse_include_directive(line: &str, source_path: &Path) -> Result<Option<PathBuf>> {
    ...
    Ok(Some(PathBuf::from(include_path)))
}
...
mod tests {
    ...
    fn load_from_str_expands_environment_placeholders_inside_strings() {
        ...
        match &config.locations[0].handler {
            crate::model::HandlerConfig::Static { body, .. } => {
                assert_eq!(body, "hello \"env\"\n");
            }
    ...
    fn load_from_str_supports_env_defaults_and_literal_dollar_escape() {
        ...
        match &config.locations[0].handler {
            crate::model::HandlerConfig::Static { body, .. } => {
                assert_eq!(body, "${rginx_test_missing}");
            }
            _ => panic!("expected static handler"),
        }
    }
    ...
    fn load_from_path_expands_relative_includes_recursively() {
        ...
        match &config.locations[0].handler {
            crate::model::HandlerConfig::Static { body, .. } => assert_eq!(body, "included body\n"),
            _ => panic!("expected static handler"),
        }
