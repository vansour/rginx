...
pub fn router() -> Router<crate::state::AppState> {
    Router::new()
        ...
        .route("/api/v1/agent/dns/snapshot", get(agent::get_dns_snapshot))
        ...
        .route("/api/v1/dns/runtime", get(dns::get_runtime_status))
        .route("/api/v1/dns/deployments", get(dns::list_deployments).post(dns::create_deployment))
        .route("/api/v1/dns/deployments/{deployment_id}", get(dns::get_deployment))
        .route("/api/v1/dns/deployments/{deployment_id}/pause", post(dns::pause_deployment))
        .route("/api/v1/dns/deployments/{deployment_id}/resume", post(dns::resume_deployment))
        .route("/api/v1/dns/deployments/{deployment_id}/rollback", post(dns::rollback_deployment))
        .route("/api/v1/dns/revisions", get(dns::list_revisions))
        .route("/api/v1/dns/revisions/drafts", get(dns::list_drafts).post(dns::create_draft))
        .route("/api/v1/dns/revisions/{revision_id}", get(dns::get_revision))
        .route(
            "/api/v1/dns/revisions/drafts/{draft_id}",
            get(dns::get_draft).put(dns::update_draft),
        )
        .route("/api/v1/dns/revisions/drafts/{draft_id}/validate", post(dns::validate_draft))
        .route("/api/v1/dns/revisions/drafts/{draft_id}/diff", get(dns::diff_draft))
        .route("/api/v1/dns/revisions/drafts/{draft_id}/publish", post(dns::publish_draft))
        .route("/api/v1/dns/simulate", post(dns::simulate_query))
