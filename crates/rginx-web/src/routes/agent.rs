...
use rginx_control_types::{
    NodeAgentDnsSnapshotResponse, NodeAgentHeartbeatRequest, NodeAgentRegistrationRequest,
    ...
    NodeAgentWriteResponse, NodeSnapshotIngestRequest, NodeSnapshotIngestResponse,
};
...
pub async fn get_dns_snapshot(
    BoundAgentGuard(identity): BoundAgentGuard,
    request_context: RequestContext,
    State(state): State<AppState>,
) -> ApiResult<Json<NodeAgentDnsSnapshotResponse>> {
    let mut response = NodeAgentDnsSnapshotResponse {
        snapshot: state
            .services()
            .dns()
            .published_snapshot_for_node(&identity.cluster_id, &identity.node_id)
            .await
            .map_err(|error| {
                ApiError::from(error).with_request_id(request_context.request_id.as_str())
            })?,
        fetched_at_unix_ms: unix_time_ms(SystemTime::now()),
        agent_token: None,
