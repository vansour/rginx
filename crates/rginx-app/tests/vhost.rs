...
#[test]
fn exact_host_beats_wildcard_host_even_when_declared_later() {
    let listen_addr = reserve_loopback_addr();
    let mut server = ServerHarness::spawn("rginx-vhost-exact-over-wildcard", |_| {
        exact_over_wildcard_config(listen_addr)
    });
    server.wait_for_http_ready(listen_addr, Duration::from_secs(5));
...
#[test]
fn more_specific_wildcard_host_beats_less_specific_wildcard_host() {
    let listen_addr = reserve_loopback_addr();
    let mut server = ServerHarness::spawn("rginx-vhost-specific-wildcard", |_| {
        specific_wildcard_priority_config(listen_addr)
    });
    server.wait_for_http_ready(listen_addr, Duration::from_secs(5));
...
fn exact_over_wildcard_config(listen_addr: SocketAddr) -> String {
    format!(
        "Config(\n    runtime: RuntimeConfig(\n        shutdown_timeout_secs: 2,\n    ),\n    server: ServerConfig(\n        listen: {:?},\n    ),\n    upstreams: [],\n    locations: [\n{ready_route}    ],\n    servers: [\n        VirtualHostConfig(\n            server_names: [\"*.example.com\"],\n            locations: [\n                LocationConfig(\n                    matcher: Exact(\"/\"),\n                    handler: Return(\n                        status: 200,\n                        location: \"\",\n                        body: Some(\"wildcard root\\n\"),\n                    ),\n                ),\n            ],\n        ),\n        VirtualHostConfig(\n            server_names: [\"api.example.com\"],\n            locations: [\n                LocationConfig(\n                    matcher: Exact(\"/\"),\n                    handler: Return(\n                        status: 200,\n                        location: \"\",\n                        body: Some(\"exact root\\n\"),\n                    ),\n                ),\n            ],\n        ),\n    ],\n)\n",
        listen_addr.to_string(),
        ready_route = READY_ROUTE_CONFIG,
    )
}

fn specific_wildcard_priority_config(listen_addr: SocketAddr) -> String {
    format!(
        "Config(\n    runtime: RuntimeConfig(\n        shutdown_timeout_secs: 2,\n    ),\n    server: ServerConfig(\n        listen: {:?},\n    ),\n    upstreams: [],\n    locations: [\n{ready_route}    ],\n    servers: [\n        VirtualHostConfig(\n            server_names: [\"*.example.com\"],\n            locations: [\n                LocationConfig(\n                    matcher: Exact(\"/\"),\n                    handler: Return(\n                        status: 200,\n                        location: \"\",\n                        body: Some(\"generic wildcard root\\n\"),\n                    ),\n                ),\n            ],\n        ),\n        VirtualHostConfig(\n            server_names: [\"*.api.example.com\"],\n            locations: [\n                LocationConfig(\n                    matcher: Exact(\"/\"),\n                    handler: Return(\n                        status: 200,\n                        location: \"\",\n                        body: Some(\"specific wildcard root\\n\"),\n                    ),\n                ),\n            ],\n        ),\n    ],\n)\n",
        listen_addr.to_string(),
        ready_route = READY_ROUTE_CONFIG,
    )
}
