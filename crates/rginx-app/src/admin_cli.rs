...
fn render_reload_active_revision(result: Option<&rginx_http::ReloadResultSnapshot>) -> String {
    result.map(|result| result.active_revision.to_string()).unwrap_or_else(|| "-".to_string())
}

fn render_reload_rollback_revision(result: Option<&rginx_http::ReloadResultSnapshot>) -> String {
    result
...
fn render_reload_tls_certificate_changes(
...
fn render_optional_string_list(values: Option<&[String]>) -> String {
    match values {
