...
### 2. 去掉 zone 级全局 `io_lock`，改为更细粒度的并发控制

当前单个 cache zone 的缓存文件读写存在明显的全局串行化特征。

- 当前问题：
  - 命中、回填、重验证、清理之间容易互相阻塞
  - zone 内热点 key 和冷 key 会共享同一层 I/O 串行瓶颈
  - 随并发上升，缓存命中路径吞吐会更早触顶
- 长期目标：
  - 将并发控制粒度收敛到 entry、hash、目录分片或等价级别
  - 避免单 zone 全局 I/O 锁成为缓存吞吐上限
  - 在不破坏一致性的前提下提高并发命中和并发回填能力

### 3. 将淘汰与 inactive cleanup 从全量扫描改为增量、低抖动后台维护
...
### 4. 补齐 range / stale / revalidate / policy 的长尾控制项
...
### 5. 建立缓存专项 benchmark、压力测试与故障注入体系
