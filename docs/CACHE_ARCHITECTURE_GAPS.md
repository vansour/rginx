...
- `shared_index` 已经不是旧的 sidecar 文件，而是带 generation 和 delta replay 能力的 SQLite 共享元数据存储
...
- 当前已经支持 zone / key / prefix / tag 级逻辑失效，失效规则写入同一份本地索引和 SQLite shared index，并在 lookup 时以 lazy drop 语义删除旧对象
...
### 4. 真正的共享热元数据层，而不只是本地热层 + 共享磁盘元数据库

`shared_index` 已经解决了跨 manager / reload 的共享元数据问题，Phase 1 也已经补上了进程内本地热元数据层；但它们组合起来，仍然不是成熟缓存引擎意义上的共享 hot metadata layer。

- 当前共享索引是 SQLite 持久化元数据库，本地 lookup 虽然已经有热 response head 和本地热访问时间，但仍依赖各进程自己的内存索引与本地热状态
- 这与 NGINX `keys_zone` 或 ATS RAM cache 的目标并不相同
- 当缓存规模继续扩大时，热 key 目录、admission count、fill lock 状态的维护仍然偏重
- 后续 NGINX-style shared memory 迁移应限定为共享热元数据层；response body 继续由磁盘 `.body` 承载，`.meta.json` 继续作为重建 shared memory 的持久来源
...
- 先把共享元数据同步改成更轻的增量模型，再考虑 shared memory；先减少 reload / sync 开销，再替换存储介质
...
- 已在 SQLite shared index 上增加变更序列，`SharedIndexOperation` 会同时更新 durable 快照表与 change log
...
- `crates/rginx-http/src/cache/shared.rs`
- `crates/rginx-http/src/cache/shared/index_file/mod.rs`
- `crates/rginx-http/src/cache/shared/index_file/schema.rs`
- `crates/rginx-http/src/cache/shared/index_file/sqlite.rs`
...
原因很直接：命中体流式化、本地热元数据层、shared metadata 增量同步、Phase 3 的后台维护重写、Phase 4 的生命周期模型，以及 Phase 5 的逻辑失效层与 proxy 边界 backend trait 都已经完成。接下来最限制 cache 继续成熟的，变成了共享热层本身、复杂 range / large-object 策略，以及“把当前第一层 invalidation / backend interface 继续向下做深”。如果这些层不继续收敛，后面的 predicate BAN、RAM tier、对象存储后端和更强策略表达力仍会耦合在当前磁盘后端细节上。
