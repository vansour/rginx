...
usage() {
    cat <<'EOF'
Usage: prepare-release.sh --tag <stable-tag> [options]
...
[[ "${TAG}" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]] || die "stable release tag must match v<major>.<minor>.<patch>"
...
[[ "${CURRENT_BRANCH}" == "main" ]] || die "stable release prep must run from the main branch"
